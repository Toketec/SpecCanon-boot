...
**PM 交付物清单**（按需选，不强制全部写完才进入 Step 2）:
...
### 3.5 Step 5 — Dev 收尾验收（AI 辅助修 bug）
...
1. **读顺序**: AGENTS.md → ADR/（架构全景 → 快速理解系统）→ docs/sprints/（确认功能意图）→ 目标模块 specs/（实现细节）
...
| `docs/sprints/` | **版本层** | 每次迭代的完整产品设计容器。PM 在每个版本中更新功能描述、场景、流程、线框图和可交互原型。**上次版本的原型和文档作为历史记录，不覆盖** |
